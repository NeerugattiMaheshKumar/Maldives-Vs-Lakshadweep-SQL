...
select sum(numberofvisitors) as total_visitors
...
select year,averagebudgetperday, besttimetovisit,sum(numberofvisitors) as total_tourists 
from destination d
join touristarrivals ta on 
ta.destinationid = d.destinationid
where year =2022
group by year, BestTimeToVisit,AverageBudgetPerDay
having total_tourists >10000;

select d.name, ta.name, d.uniquefeatures from
destination d join touristattractions ta on 
ta.DestinationID =d. DestinationID
where d.name = "lakshadweep";

select d.name,numberoftourists as indiantourists
from indiantourists It
join destination d  on d.DestinationID= It.DestinationID
where year = "2021" and d.name= "maldives";

select ta.destinationid, d.name,
case when ta.destinationid =1 then (d.averagebudgetperday*ta.numberofvisitors)*10
end as highest_expected_tourismrevenue
from destination d join touristarrivals ta on ta.DestinationID = d.DestinationID
where year =2022 and d.name = 'Maldives';
